...
def sourceGraph(fig_d, colors_, canvas_area, tool_area,x_min=None,x_max=None,**kwargs):
    ...
    if   radVar.get() == 0: pass
    elif radVar.get() == 1: a.grid()
    #--------------------------------------------------
...
def wdgt(widget,tabName,text_,textv_,he_,wi_,command_,c_,r_,py_,px_,cs_,rs_,stik_,**kwargs):
	if widget=='btn':
		ttk.Button(tabName, text=text_, command=command_).grid(column=c_, row=r_, pady=py_, padx=px_, columnspan=cs_, rowspan=rs_,sticky=stik_)
	elif widget=='label':
		ttk.Label(tabName, text=text_).grid(column=c_, row=r_, pady=py_, padx=px_, columnspan=cs_, rowspan=rs_,sticky=stik_)
...
color_chosen['values'] = ('Red','Blue','Green','Cyan','Magenta','Orange','Yellow','Olive','Gray','Purple','Brown','Black')
